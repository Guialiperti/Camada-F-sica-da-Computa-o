...
class enlace(object):
    ...
    def client_transmission(self,payload):
        ...
        for i in range(len(lista_pacotes)):
            sync_package4 = self.tx.cria_package(lista_pacotes[i], 4, i+1, len(lista_pacotes), 0)
            ...
            while True:
                ...
                run_time = time.time() - timer
                if run_time > 4:
                    print("Tipo 5 ou 6 NAO recebido ------> Reenviando tipo 4 | {0} / {1}  Tamanho:{2} bytes".format(i+1, total_pacotes, len(lista_pacotes[i])))
                    self.tx.sendBuffer(sync_package4)
                    timer = time.time()
    ...
    def server_transmission(self):
        ...
        while True:
            ...
            if tipo == 4 and not encoding:
                ...
                sync1 = True
                print("recebeu tipo 4 | {0}/{1} CORRETO, ENVIA 5".format(n_pacote,total_pacotes))
